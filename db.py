"""Lakebase data-access layer for the support ticket Databricks App."""
...
class OAuthConnection(psycopg.Connection):
    """Creates each PostgreSQL connection with a fresh Lakebase OAuth token."""
    ...
    def connect(cls, conninfo: str = "", **kwargs: Any) -> "OAuthConnection":
        ...
        if not endpoint_name:
            raise RuntimeError(
                "ENDPOINT_NAME is missing. Add the Lakebase Autoscaling resource "
                "to the app with resource key 'postgres'."
            )
...
def _connection_info() -> str:
    ...
    if missing:
        raise RuntimeError(
            "Missing Lakebase environment variable(s): " + ", ".join(missing)
        )
    ...
    connection_params = {
        ...
        "appname": os.getenv("PGAPPNAME", "lakebase-support-app"),
    }

    # Used only for optional local native-password authentication.
    if os.getenv("PGPASSWORD"):
        connection_params["password"] = os.environ["PGPASSWORD"]

    return make_conninfo(**connection_params)


def get_pool() -> ConnectionPool:
    """Return a process-wide connection pool."""
    global _pool

    if _pool is None:
        endpoint_name = os.getenv("ENDPOINT_NAME")
        connection_class = OAuthConnection if endpoint_name else psycopg.Connection
        ...
        _pool = ConnectionPool(
            ...
            max_size=8,
            max_idle=300,
            # Recycle connections before the one-hour OAuth credential lifetime.
            max_lifetime=3300,
            timeout=30,
            open=False,
        )
