...
from massive_client import MassiveApiError, is_massive_configured, test_massive_connection
...
try:
    ...
    with st.expander("Connection checklist"):
        st.markdown(
            """
            1. Create the secret scope by running `python setup_secrets.py`.
            2. Add secret `lakebase-support-app/lakebase-url` as an App resource.
            3. Set its custom resource key to **`lakebase_url_secret`**.
            4. Confirm the password role has `CONNECT` and `CREATE` on the database.
            5. Redeploy the app after adding the secret resource.
            """
        )
    st.stop()
...
with st.sidebar:
    ...
    with st.expander("Connection status"):
        st.success("Lakebase connected through the secret-backed URL.")
        if is_massive_configured():
            st.success("Massive API secret is loaded.")
            symbol = st.text_input(
                "Symbol for API test",
                value="AAPL",
                max_chars=10,
                key="massive_test_symbol",
            )
            if st.button("Test Massive API", use_container_width=True):
                try:
                    result = test_massive_connection(symbol)
                    close_value = result.get("previous_close")
                    close_text = f" Previous close: {close_value}." if close_value is not None else ""
                    st.success(
                        f"Massive connection succeeded for {result['symbol']}." + close_text
                    )
                except MassiveApiError as exc:
                    st.error(str(exc))
        else:
            st.warning(
                "Massive secret is not loaded. Add it as an App resource with "
                "key `massive_api_key_secret`."
            )
